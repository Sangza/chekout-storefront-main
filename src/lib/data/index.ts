...
import insertQuery from "./query-with-store-id"
...
export const getProductByHandle = cache(async function (
  ...
  const product = await medusaClient.products
    .list({ handle, q: insertQuery(store_id) }, headers)
...
export const getStoreInformation = async ({ name }: { name: string }) => {
  const { data, status } = await chekoutRequestJSON("GET", get_store_info(name))
  if (status === 500) throw new Error("Server Error")
  else if (status === 200) return data.store as StoreInfo
  else return false
}
